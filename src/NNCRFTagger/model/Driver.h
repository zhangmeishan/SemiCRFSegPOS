...
public:
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			//}
			cost += _model_params._loss.loss(getPNodes(_pcg->output, seq_size), example.m_labels, _eval, example_num);
	...
	inline void predict(const vector<Feature>& features, vector<int>& results) {
		...
		//}
		_model_params._loss.predict(getPNodes(_pcg->output, seq_size), results);
	}
	...
	inline dtype cost(const Example& example){
		...
		//}
		cost += _model_params._loss.cost(getPNodes(_pcg->output, seq_size), example.m_labels, 1);
